	...
	if os.path.isfile(sys.argv[1]):
		...
		for list in filelist:
			...
			for  dnskey in dns.iterkeys():
				pcapsecondleveldns = {}
				dnslistkey = ''
				if len(dnskey.split('.')) == 2:
					dnslistkey = dnskey.split('.')[-2]
				else:
					dnslistkey = dnskey.split('.')[-3]+'.'+dnskey.split('.')[-2]
				if (dnslistkey) not in secondleveldns:
					dnslist = []
					dnslist.append(dnskey)
					pcapsecondleveldns[list] = dnslist
					secondleveldns[dnslistkey] = pcapsecondleveldns
				else:
					if list not in secondleveldns[dnslistkey].keys():
						dnslist = []
						dnslist.append(dnskey)
						secondleveldns[dnslistkey][list] = dnslist
					else:
						secondleveldns[dnslistkey][list].append(dnskey)
		for (k,v) in secondleveldns.iteritems():
			print '-----------------------------'
			print "first level Domain: %s" % k
			print "Total: %d" % len(v)
